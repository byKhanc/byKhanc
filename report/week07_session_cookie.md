...
2. **서버 인증 및 JWT 생성**  
   ...
   - JWT는 **암호화 or 서명**되어 위조 방지됨  
   - 서버는 `Set-Cookie` 헤더 또는 JS에서 `localStorage`에 JWT 저장

3. **요청 시 JWT 포함**  
   - 클라이언트는 JWT를 `Authorization: Bearer` 헤더 또는 쿠키에 실어 전송  
   - 서버는 JWT를 **복호화 및 검증**하여 사용자 확인

4. **로그아웃 시 토큰 삭제** (클라이언트 측에서)

🔐 **장점**: 서버 상태 불필요 → 스케일링 용이, API 서버에 적합  
⚠️ **단점**: 토큰 탈취 시 위험, 토큰 만료/갱신 정책 필수
